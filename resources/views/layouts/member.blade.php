    ...
    <aside class="w-64 bg-teal-800 text-white flex flex-col shadow-xl">
        <div class="h-16 flex items-center px-6 border-b border-teal-700">
            <a href="{{ route('dashboard') }}"
                class="flex items-center gap-3 w-full hover:opacity-80 transition-opacity" title="Go to Dashboard">
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-3 space-y-1">
            ...
            <a href="{{ route('member.dashboard') }}"
                class="flex items-center px-3 py-2.5 rounded-xl transition-colors {{ request()->routeIs('member.dashboard') ? 'bg-teal-700 font-medium' : 'hover:bg-teal-700/50 text-teal-50' }}">
            ...
            <a href="{{ route('member.deadlines') }}"
                class="flex items-center px-3 py-2.5 rounded-xl transition-colors {{ request()->routeIs('member.deadlines') ? 'bg-teal-700 font-medium' : 'hover:bg-teal-700/50 text-teal-50' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('member.chat') }}"
                class="flex items-center px-3 py-2.5 rounded-xl transition-colors {{ request()->routeIs('member.chat') ? 'bg-teal-700 font-medium' : 'hover:bg-teal-700/50 text-teal-50' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('member.profile') }}"
                class="flex items-center px-3 py-2.5 rounded-xl transition-colors {{ request()->routeIs('member.profile') ? 'bg-teal-700 font-medium' : 'hover:bg-teal-700/50 text-teal-50' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('member.settings') }}"
                class="flex items-center px-3 py-2.5 rounded-xl transition-colors {{ request()->routeIs('member.settings') ? 'bg-teal-700 font-medium' : 'hover:bg-teal-700/50 text-teal-50' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="p-4 border-t border-teal-700">
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 bg-teal-600 rounded-full flex items-center justify-center text-white font-semibold">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="p-1.5 text-teal-200 hover:text-white rounded-lg hover:bg-teal-700 transition-colors"
                        title="Logout">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header class="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-6 z-10 shadow-sm">
            ...
            <div class="flex items-center gap-5">
                <!-- Notifications -->
                @php
                    $memberUnreadCount = \App\Models\Activity::where(function($query) {
                        $query->whereHas('task', function($q) {
                            $q->where('user_id', auth()->id());
                        })->orWhereHas('task.project', function($q) {
                            $q->whereHas('members', function($m) {
                ...
                <a href="{{ route('member.notifications.index') }}"
                    class="relative p-2 text-gray-400 hover:text-teal-600 transition-colors rounded-full hover:bg-gray-100" title="Notifications">
                ...
                <a href="{{ route('member.profile') }}" class="flex items-center gap-2 cursor-pointer">
                    <div
                        class="w-8 h-8 bg-teal-100 rounded-full flex items-center justify-center text-teal-700 font-semibold border border-teal-200 shadow-sm">
        ...
        <main class="flex-1 overflow-y-auto bg-gray-50/50 p-6">
            ...
            @if(session('success_message'))
                <div
                    class="mb-6 bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-2xl flex items-center shadow-sm">
            ...
            @if(session('error_message'))
                <div
                    class="mb-6 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-2xl flex items-center shadow-sm">
