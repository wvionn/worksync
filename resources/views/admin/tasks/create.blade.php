...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Assign a new task to your team</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.tasks.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <select name="project_id" id="project_id" required onchange="onProjectChange()"
                    ...
                    @foreach($projects as $project)
                        <option value="{{ $project->id }}" data-members="{{ json_encode($project->members->pluck('id')) }}" {{ request('project_id') == $project->id ? 'selected' : '' }}>
                            {{ $project->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" placeholder="Enter task details/description..."
                ...
                <div>
                    ...
                    <select name="priority" id="priority" required
                        ...
                        @foreach($priorityOptions as $option)
                            <option value="{{ $option }}" {{ $option === 'medium' ? 'selected' : '' }}>{{ ucfirst($option) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Assignee -->
                <div>
                    <label for="user_id" class="block text-sm font-semibold text-gray-700 mb-2">Assign To (Project Member)</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="milestone_id" id="milestone_id"
                        ...
                        @foreach($milestones as $m)
                            <option value="{{ $m->id }}" data-project="{{ $m->project_id }}">{{ $m->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex items-center pt-8">
                    <input type="checkbox" name="is_blocked" id="is_blocked" value="1" onchange="toggleBlockerText()"
            ...
            <!-- Blocker description -->
            <div id="blocker_desc_container" class="hidden">
                ...
                <textarea name="blocker_description" id="blocker_description" rows="2" placeholder="Explain why this task is blocked..."
                    class="w-full px-4 py-2 border border-red-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 bg-white text-gray-900"></textarea>
            </div>
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <label class="flex items-center gap-2 px-2.5 py-1 bg-white rounded-md border border-gray-200 cursor-pointer hover:bg-gray-100">
                        <input type="checkbox" name="label_ids[]" value="{{ $lbl->id }}" class="text-blue-600 rounded">
