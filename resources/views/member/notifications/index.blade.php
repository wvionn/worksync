...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Updates and tasks assigned to you</p>
        </div>
        
        @if(($unreadCount ?? 0) > 0)
        <form method="POST" action="{{ route('member.notifications.markAllRead') }}">
            @csrf
            <button type="submit" class="px-4 py-2 bg-teal-700 text-white rounded-lg text-sm font-semibold hover:bg-teal-800 transition-colors">
                Mark all as read
            </button>
        </form>
        @else
    ...
    <div class="bg-white rounded-xl border border-gray-200 divide-y divide-gray-200 shadow-sm overflow-hidden">
        ...
        <div @if(!$activity->is_read) onclick="document.getElementById('mark-read-{{ $activity->id }}').submit();" @endif 
             class="p-4 hover:bg-gray-50 transition-colors {{ $activity->is_read ? 'opacity-75' : 'bg-teal-50/20 cursor-pointer font-medium' }}">
            
            @if(!$activity->is_read)
            <form id="mark-read-{{ $activity->id }}" method="POST" action="{{ route('member.notifications.markRead', $activity) }}" class="hidden">
                @csrf
                @method('PATCH')
            </form>
            @endif
            ...
            <div class="flex items-start gap-4">
                <div class="w-10 h-10 {{ $activity->is_read ? 'bg-gray-100 text-gray-500' : 'bg-teal-100 text-teal-700' }} rounded-full flex items-center justify-center flex-shrink-0">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between gap-4">
                        ...
                        @if(!$activity->is_read)
                        <span class="px-2 py-0.5 text-[9px] font-extrabold uppercase rounded bg-teal-100 text-teal-800 flex-shrink-0">
                            New
                        </span>
                        @endif
                    </div>
